...
import com.ird.faa.service.chercheur.facade.formulaire.FormationContinueCommanditaireChercheurService;
...
@Service
public class FormationContinueCommanditaireChercheurServiceImpl extends AbstractServiceImpl<FormationContinueCommanditaire> implements FormationContinueCommanditaireChercheurService {
...
FormationContinueCommanditaire foundedFormationContinueCommanditaire = findById(formationContinueCommanditaire.getId());
if(foundedFormationContinueCommanditaire==null) return null;
else{
...
@Override
public List<FormationContinueCommanditaire> save(List<FormationContinueCommanditaire> formationContinueCommanditaires){
...
public int delete(FormationContinueCommanditaire formationContinueCommanditaire){
    if(formationContinueCommanditaire.getId()==null) return -1;
    FormationContinueCommanditaire foundedFormationContinueCommanditaire = findById(formationContinueCommanditaire.getId());
    if(foundedFormationContinueCommanditaire==null) return -1;
formationContinueCommanditaireDao.delete(foundedFormationContinueCommanditaire);
return 1;
}


public List<FormationContinueCommanditaire> findByCriteria(FormationContinueCommanditaireVo formationContinueCommanditaireVo){
...
String query = "SELECT o FROM FormationContinueCommanditaire o where 1=1 ";
    ...
    if(formationContinueCommanditaireVo.getCommanditaireVo()!=null){
        query += SearchUtil.addConstraint( "o", "commanditaire.id","=",formationContinueCommanditaireVo.getCommanditaireVo().getId());
            query += SearchUtil.addConstraint( "o", "commanditaire.code","LIKE",formationContinueCommanditaireVo.getCommanditaireVo().getCode());
    }

    if(formationContinueCommanditaireVo.getFormationContinueVo()!=null){
        query += SearchUtil.addConstraint( "o", "formationContinue.id","=",formationContinueCommanditaireVo.getFormationContinueVo().getId());
    }

    if(formationContinueCommanditaireVo.getPaysVo()!=null){
        query += SearchUtil.addConstraint( "o", "pays.id","=",formationContinueCommanditaireVo.getPaysVo().getId());
            query += SearchUtil.addConstraint( "o", "pays.code","LIKE",formationContinueCommanditaireVo.getPaysVo().getCode());
    }
...
return entityManager.createQuery(query).getResultList();
    ...
    private void findCommanditaire(FormationContinueCommanditaire formationContinueCommanditaire){
        Commanditaire loadedCommanditaire =commanditaireService.findByIdOrCode(formationContinueCommanditaire.getCommanditaire());
    ...
    private void findFormationContinue(FormationContinueCommanditaire formationContinueCommanditaire){
        ...
        if(formationContinueCommanditaire.getFormationContinue() != null && formationContinueCommanditaire.getFormationContinue().getId() !=null)
        loadedFormationContinue =formationContinueService.findById(formationContinueCommanditaire.getFormationContinue().getId());
