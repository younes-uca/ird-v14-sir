...
@Service
public class InstrumentIrdChercheurServiceImpl extends AbstractServiceImpl<InstrumentIrd>
		implements InstrumentIrdChercheurService {
	...
	public int deleteById(Long id) {
		if (instrumentIrdDao.findById(id) == null)
			return 0;
		else {
	...
	public List<InstrumentIrd> findByCriteria(InstrumentIrdVo instrumentIrdVo) {
		...
		query += SearchUtil.addConstraintMinMaxDate("o", "dateArchivage", instrumentIrdVo.getDateArchivageMin(),
				instrumentIrdVo.getDateArchivageMax());
		if (instrumentIrdVo.getTypeInstrumentIrdVo() != null) {
			query += SearchUtil.addConstraint("o", "typeInstrumentIrd.id", "=",
					instrumentIrdVo.getTypeInstrumentIrdVo().getId());
			query += SearchUtil.addConstraint("o", "typeInstrumentIrd.code", "LIKE",
					instrumentIrdVo.getTypeInstrumentIrdVo().getCode());
		}
	...
	private void findTypeInstrumentIrd(InstrumentIrd instrumentIrd) {
		TypeInstrumentIrd loadedTypeInstrumentIrd = typeInstrumentIrdService
				.findByIdOrCode(instrumentIrd.getTypeInstrumentIrd());
	...
	@Override
	public List<List<InstrumentIrd>> getToBeSavedAndToBeDeleted(List<InstrumentIrd> oldList,
			List<InstrumentIrd> newList) {
		return super.getToBeSavedAndToBeDeleted(oldList, newList);
	}
	
	@Override
	public List<InstrumentIrd> findByTypeInstuments(List<TypeInstrumentIrd> TypeInstrumentIrd) {
		String query = "SELECT o FROM InstrumentIrd o where o.typeInstrumentIrd in (:typesInstruments)";
        return entityManager.createQuery(query).setParameter("typesInstruments",TypeInstrumentIrd).getResultList();
	}
