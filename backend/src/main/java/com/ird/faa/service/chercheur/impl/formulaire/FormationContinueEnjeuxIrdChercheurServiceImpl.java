...
import com.ird.faa.service.chercheur.facade.formulaire.FormationContinueEnjeuxIrdChercheurService;
...
@Service
public class FormationContinueEnjeuxIrdChercheurServiceImpl extends AbstractServiceImpl<FormationContinueEnjeuxIrd> implements FormationContinueEnjeuxIrdChercheurService {
...
if(foundedFormationContinueEnjeuxIrd==null) return null;
else{
...
public int delete(FormationContinueEnjeuxIrd formationContinueEnjeuxIrd){
    if(formationContinueEnjeuxIrd.getId()==null) return -1;
    FormationContinueEnjeuxIrd foundedFormationContinueEnjeuxIrd = findById(formationContinueEnjeuxIrd.getId());
    if(foundedFormationContinueEnjeuxIrd==null) return -1;
...
String query = "SELECT o FROM FormationContinueEnjeuxIrd o where 1=1 ";
    ...
    if(formationContinueEnjeuxIrdVo.getEnjeuxIrdVo()!=null){
        query += SearchUtil.addConstraint( "o", "enjeuxIrd.id","=",formationContinueEnjeuxIrdVo.getEnjeuxIrdVo().getId());
            query += SearchUtil.addConstraint( "o", "enjeuxIrd.code","LIKE",formationContinueEnjeuxIrdVo.getEnjeuxIrdVo().getCode());
    }

    if(formationContinueEnjeuxIrdVo.getFormationContinueVo()!=null){
        query += SearchUtil.addConstraint( "o", "formationContinue.id","=",formationContinueEnjeuxIrdVo.getFormationContinueVo().getId());
    }
...
return entityManager.createQuery(query).getResultList();
    ...
    private void findFormationContinue(FormationContinueEnjeuxIrd formationContinueEnjeuxIrd){
        ...
        if(formationContinueEnjeuxIrd.getFormationContinue() != null && formationContinueEnjeuxIrd.getFormationContinue().getId() !=null)
        loadedFormationContinue =formationContinueService.findById(formationContinueEnjeuxIrd.getFormationContinue().getId());
