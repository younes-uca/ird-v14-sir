...
@Service
public class ZoneGeographiqueChercheurServiceImpl extends AbstractServiceImpl<ZoneGeographique> implements ZoneGeographiqueChercheurService {
...
if(foundedZoneGeographique==null) return null;
else{
...
public int delete(ZoneGeographique zoneGeographique){
    if(zoneGeographique.getCode()==null) return -1;

    ZoneGeographique foundedZoneGeographique = findByCode(zoneGeographique.getCode());
    if(foundedZoneGeographique==null) return -1;
...
String query = "SELECT o FROM ZoneGeographique o where 1=1 ";
        ...
        query += SearchUtil.addConstraintDate( "o", "dateCreation","=",zoneGeographiqueVo.getDateCreation());
            query += SearchUtil.addConstraintMinMaxDate("o","dateArchivage",zoneGeographiqueVo.getDateArchivageMin(),zoneGeographiqueVo.getDateArchivageMax());
            query += SearchUtil.addConstraintMinMaxDate("o","dateCreation",zoneGeographiqueVo.getDateCreationMin(),zoneGeographiqueVo.getDateCreationMax());
return entityManager.createQuery(query).getResultList();
}
